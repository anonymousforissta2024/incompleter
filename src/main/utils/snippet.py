...
class Snippet:
    ...
    def compute_latest_coverage(self, proc_q) -> Tuple[float, float]:
        ...
        try:
            ...
            with open(os.path.join(DATA_DIR, 'sample_inputs', 'input1.txt'), 'r') as input_file:
                ...
                if os.path.exists(os.path.join(self.tmp_dir, 'coverage.json')):
                    ...
                    if 'files' in cov_json.keys() and self.snippet_name in cov_json['files'].keys() and 'summary' in cov_json['files'][self.snippet_name].keys():
                        ...
                        cov_summary = {}
                        cov_summary['covered_lines'] = summary['covered_lines']
                        cov_summary['num_statements'] = summary['num_statements']
                        cov_summary['num_branches'] = summary['num_branches']
                        cov_summary['num_partial_branches'] = summary['num_partial_branches']
                        cov_summary['covered_branches'] = summary['covered_branches']
                        if cov_summary is not None:
                            stmt_cov = cov_summary['covered_lines']/cov_summary['num_statements']
                            if stmt_cov is not None:
                                stmt_cov = stmt_cov/100
                            if cov_summary['num_branches'] == 0:
                                br_cov = None # 1.0
                            else:
                                br_cov = cov_summary['covered_branches']/(cov_summary['num_branches'] - cov_summary['num_partial_branches'])
